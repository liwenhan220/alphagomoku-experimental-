...
board = Checkerboard(GRID_SIZE, GRID_SIZE, win_count=5, history_rec=HISTORY_REC)
...
if AI_PLAYER == 0:
    ...
    AI_TURN = 100
net = Network(2 * HISTORY_REC + 1, NUM_FILTERS, NUM_BLOCKS, WIDTH, HEIGHT)
net.load_state_dict(torch.load(get_latest_model(MODEL_DIR)))
net.eval()
# net = AlphaZeroNetwork(board_size=WIDTH)
mcts = MCTS(board, net, 'cuda' if torch.cuda.is_available() else 'cpu')
mcts.reset()
...
def main():
    ...
    while running:
        ...
        if not game_over:
            ...
            print(f'MCTS_visits:{sum(mcts.root.Ns)}, win_rate:{max((np.where(board.valid_actions.flatten(), mcts.root.Qs, float('-inf')) + 1)/2)}')
            # print(mcts.root.Ns.reshape(WIDTH, HEIGHT))

        if (board.currentplayer == AI_TURN or AI_TURN == 2) and not game_over:
            if last_t is None:
                last_t = time.time()
            elif time.time() - last_t >= MAX_THINKING_TIME:
                # print(f'MCTS_visits:{sum(mcts.root.Ns)}, win_rate:{max((np.where(board.valid_actions.flatten(), mcts.root.Qs, float('-inf')) + 1)/2)}')
                best_a = mcts.recommend_action()
                # print(mcts.root.Ns.reshape(board.height, board.width))
                x, y = mcts.A2xy(best_a)
                result, winner = mcts.step(x, y)
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    running = False
                elif event.type == pygame.MOUSEBUTTONDOWN and board.currentplayer:
                    if reset_button_rect.collidepoint(event.pos):
                        ...
                        if mouse_y < SCREEN_SIZE:  # Ensure clicks are within the board area
                            ...
                            if board.board[grid_y][grid_x] == 0:
                                result, winner = mcts.step(grid_y, grid_x)
                                # print(board.get_state()[0])
                                if result:
                                    game_over = True
                                    print(f"Player {'Black' if winner == board.black else 'White'} wins!")
